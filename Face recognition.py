...
def main():
    ...
    init.camera_resolution = sl.RESOLUTION.RESOLUTION_HD1080  # 初始化HD为1080
    init.depth_mode = sl.DEPTH_MODE.DEPTH_MODE_PERFORMANCE  # 初始化性能
    init.coordinate_units = sl.UNIT.UNIT_METER  # 初始化距离为米
    if len(sys.argv) >= 2:
    ...
    while key != 113:
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            faces = detector.detectMultiScale(gray, 1.3, 5)
            for (x, y, w, h) in faces:
                cv2.rectangle(image_ocv, (x, y), (x + w, y + h), (255, 0, 0),2)
